...
impl LayoutCalculator {
    ...
    /// Test helper: Simulates time passing to trigger time-based cleanup
    /// This allows testing the actual cleanup mechanism used in production
    #[cfg(test)]
    pub fn simulate_time_passing(&mut self, seconds: u64) {
        // Manually advance the last cleanup time to simulate time passing
        self.last_cache_cleanup = self.last_cache_cleanup - Duration::from_secs(seconds);
    }
...
mod tests {
    ...
    #[test]
    fn test_time_based_cache_cleanup() {
        let mut calculator = LayoutCalculator::new();
        ...
        // Use a value greater than the cleanup interval (30 seconds)
        calculator.simulate_time_passing(35);

        // Trigger cleanup by calling calculate_layout (which calls cleanup_cache_if_needed)
        calculator.calculate_layout((100, 30));

        let stats_after = calculator.get_cache_stats();

        // Verify that cleanup was triggered by time-based mechanism
        // The cache should have fewer entries after time-based cleanup
        assert!(stats_after.total_entries < stats_before.total_entries);

        // Verify that the cleanup timestamp was updated
        // This ensures the time-based mechanism is working correctly
        assert!(calculator.last_cache_cleanup.elapsed() < Duration::from_secs(1));
    }
