...
impl ContentAdapter {
    ...
    fn get_extended_team_name(team_name: &str) -> String {
        // Use externalized team name mappings from constants
        crate::constants::get_extended_team_name(team_name).to_string()
    }

    /// Gets team record information (placeholder for future implementation)
    fn get_team_record(_team_name: &str) -> Option<String> {
        // TODO: Implement team record functionality
        //
        // This function should fetch and return team season records (e.g., "15-8-2" for wins-losses-ties)
        // from the Liiga API or a separate statistics endpoint.
        //
        // Implementation requirements:
        // 1. Add team statistics API endpoint to data_fetcher/api.rs
        // 2. Create TeamRecord model in data_fetcher/models.rs
        // 3. Implement caching for team records in data_fetcher/cache.rs
        // 4. Add configuration option to enable/disable team records display
        // 5. Update ExtendedTeamInfo struct to include record formatting
        //
        // Related issues:
        // - Feature: Team season statistics display
        // - Enhancement: Team standings integration
        // - API: Liiga team statistics endpoint integration
        //
    ...
    fn translate_goal_types(goal_types: &[String]) -> String {
        let translations: Vec<String> = goal_types
            .iter()
            .map(|gt| crate::constants::translate_goal_type(gt).to_string())
            .collect();
    ...
    pub fn adapt_content_progressively(
        ...
        // Calculate content complexity and determine optimal detail level
        ...
        Self::apply_content_priorities(&mut adapted_content, &priority, enhanced_game, available_width, available_height);

        // Recalculate estimated height
        Self::recalculate_content_height(&mut adapted_content);

        adapted_content
    }

    /// Applies content prioritization to adapted content
    fn apply_content_priorities(
        adapted_content: &mut AdaptedGameContent,
        priority: &ContentPriority,
        enhanced_game: &EnhancedGameDisplay,
        available_width: u16,
        available_height: u16,
    ) {
        // Apply goal details prioritization
        Self::apply_goal_details_priority(adapted_content, priority, available_height);

        // Apply team names prioritization
        Self::apply_team_names_priority(adapted_content, priority, enhanced_game, available_width);

        // Apply time display prioritization
        Self::apply_time_display_priority(adapted_content, priority, enhanced_game);
    }

    /// Applies goal details prioritization
    fn apply_goal_details_priority(
        adapted_content: &mut AdaptedGameContent,
        priority: &ContentPriority,
        available_height: u16,
    ) {
    ...
    /// Applies team names prioritization
    fn apply_team_names_priority(
        adapted_content: &mut AdaptedGameContent,
        priority: &ContentPriority,
        enhanced_game: &EnhancedGameDisplay,
        available_width: u16,
    ...
    /// Applies time display prioritization
    fn apply_time_display_priority(
        adapted_content: &mut AdaptedGameContent,
        priority: &ContentPriority,
        enhanced_game: &EnhancedGameDisplay,
    ) {
    ...
    /// Recalculates the estimated height of adapted content
    fn recalculate_content_height(adapted_content: &mut AdaptedGameContent) {
        let base_height = 1;
    ...
    pub fn scale_content_to_fit(
        ...
        if content.goal_lines.len() > max_goal_lines {
            ...
            Self::add_truncation_indicator(&mut content.goal_lines);
        }
    ...
    /// Adds truncation indicator to the last goal line if content was truncated
    fn add_truncation_indicator(goal_lines: &mut Vec<String>) {
        if goal_lines.is_empty() {
            return;
        }

        let last_index = goal_lines.len() - 1;
        if let Some(last_line) = goal_lines.get_mut(last_index) {
            if last_line.len() > 3 {
