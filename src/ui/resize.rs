...
impl ResizeHandler {
    ...
    /// Test helper: Simulates consecutive failures by directly setting the failure count
    /// This is used for testing the reset functionality without relying on actual terminal detection
    #[cfg(test)]
    pub fn simulate_failures(&mut self, failure_count: u32) {
        // Directly set the failure count for testing purposes
        // This simulates the state after multiple detection failures
        self.consecutive_failures = failure_count;
    }
}
...
mod tests {
    ...
    fn test_manual_known_good_size_setting() {
        ...
        handler.simulate_failures(5); // Simulate failures using the test helper
        assert_eq!(handler.consecutive_failures(), 5);
        handler.set_known_good_size((100, 30));
        assert_eq!(handler.consecutive_failures(), 0);
    }
}
