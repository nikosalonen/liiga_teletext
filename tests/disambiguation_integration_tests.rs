...
use liiga_teletext::data_fetcher::models::{GoalEvent, GoalEventData, Player, ScheduleGame, ScheduleTeam};
use liiga_teletext::data_fetcher::player_names::{format_with_disambiguation, format_for_display_with_first_initial, DisambiguationContext};
...
fn create_realistic_player_data() -> (Vec<(i64, String, String)>, Vec<(i64, String, String)>) {
...
async fn test_end_to_end_disambiguation_with_real_world_names() {
    ...
    let goal_events = process_goal_events_with_disambiguation(&schedule_game, &home_players, &away_players);
    ...
    let koivu_goals: Vec<_> = goal_events.iter()
    ...
    let mikko_koivu_goal = goal_events.iter()
        .find(|event| event.scorer_player_id == 1001)
        .expect("Should find Mikko Koivu goal");
    assert_eq!(mikko_koivu_goal.scorer_name, "Koivu M.", "Mikko Koivu should be disambiguated as 'Koivu M.'");

    let saku_koivu_goal = goal_events.iter()
        .find(|event| event.scorer_player_id == 1002)
        .expect("Should find Saku Koivu goal");
    assert_eq!(saku_koivu_goal.scorer_name, "Koivu S.", "Saku Koivu should be disambiguated as 'Koivu S.'");

    // Check that Granlund player is disambiguated (away team)
    let granlund_goal = goal_events.iter()
        .find(|event| event.scorer_player_id == 2005)
        .expect("Should find Markus Granlund goal");
    assert_eq!(granlund_goal.scorer_name, "Granlund M.", "Markus Granlund should be disambiguated as 'Granlund M.'");

    // Check that Jokinen players are disambiguated
    let jokinen_goal = goal_events.iter()
        .find(|event| event.scorer_player_id == 1007)
        .expect("Should find Jussi Jokinen goal");
    assert_eq!(jokinen_goal.scorer_name, "Jokinen J.", "Jussi Jokinen should be disambiguated as 'Jokinen J.'");
...
async fn test_complete_data_flow_api_to_display() {
    ...
    assert_eq!(home_disambiguated.get(&1001), Some(&"Koivu M.".to_string()), "Mikko Koivu should be disambiguated");
    assert_eq!(home_disambiguated.get(&1002), Some(&"Koivu S.".to_string()), "Saku Koivu should be disambiguated");
    assert_eq!(home_disambiguated.get(&1003), Some(&"Selänne".to_string()), "Selänne should not be disambiguated");
    assert_eq!(home_disambiguated.get(&1006), Some(&"Jokinen O.".to_string()), "Olli Jokinen should be disambiguated");
    assert_eq!(home_disambiguated.get(&1007), Some(&"Jokinen J.".to_string()), "Jussi Jokinen should be disambiguated");

    assert_eq!(away_disambiguated.get(&2004), Some(&"Granlund M.".to_string()), "Mikael Granlund should be disambiguated");
    assert_eq!(away_disambiguated.get(&2005), Some(&"Granlund M.".to_string()), "Markus Granlund should be disambiguated");
    assert_eq!(away_disambiguated.get(&2001), Some(&"Laine".to_string()), "Laine should not be disambiguated");
    ...
    let game_data = GameResultData {
        ...
        goal_events: vec![
            ...
            GoalEventData {
                ...
                scorer_name: "Granlund M.".to_string(), // Should be disambiguated
                minute: 12,
                home_team_score: 1,
                away_team_score: 1,
                is_winning_goal: false,
...
async fn test_goal_events_show_correct_disambiguated_scorer_names() {
    ...
    // Process goal events with disambiguation
    let processed_events = process_goal_events_with_disambiguation(&schedule_game, &home_players, &away_players);

    // Verify all goal events have correct disambiguated names
    assert_eq!(processed_events.len(), 4, "Should have 4 processed goal events");

    // Check Mikko Koivu goal
    let mikko_goal = processed_events.iter()
        .find(|event| event.scorer_player_id == 101)
        .expect("Should find Mikko Koivu goal");
    assert_eq!(mikko_goal.scorer_name, "Koivu M.", "Mikko Koivu should be 'Koivu M.'");
    ...
    let saku_goal = processed_events.iter()
        .find(|event| event.scorer_player_id == 102)
        .expect("Should find Saku Koivu goal");
    assert_eq!(saku_goal.scorer_name, "Koivu S.", "Saku Koivu should be 'Koivu S.'");
    assert_eq!(saku_goal.minute, 15, "Goal should be at minute 15");
    assert_eq!(saku_goal.goal_types, vec!["YV"], "Should have power play goal type");
    assert!(saku_goal.video_clip_url.is_some(), "Should have video clip URL");

    // Check Kimmo Koivu goal
    let kimmo_goal = processed_events.iter()
        .find(|event| event.scorer_player_id == 103)
        .expect("Should find Kimmo Koivu goal");
    assert_eq!(kimmo_goal.scorer_name, "Koivu K.", "Kimmo Koivu should be 'Koivu K.'");
...
async fn test_performance_impact_with_large_datasets() {
    ...
    let last_names = vec!["Koivu", "Selänne", "Kurri", "Granlund", "Jokinen", "Laine", "Barkov", "Aho"];
    let first_names = vec!["Mikko", "Saku", "Teemu", "Jari", "Ville", "Olli", "Jussi", "Patrik", "Sebastian", "Aleksander"];
    ...
    for i in 0..20 {
        let player_id = if i % 2 == 0 { 1000 + (i as i64) } else { 2000 + (i as i64) };
        goal_events.push(GoalEvent {
            scorer_player_id: player_id,
            log_time: format!("2024-01-15T18:{}:00Z", 30 + i),
            game_time: (i as i32) * 180, // Every 3 minutes
            period: (i / 7) + 1,
            event_id: i as i32 + 1,
            ...
            goal_types: if i % 3 == 0 { vec!["YV".to_string()] } else { vec![] },
            assistant_player_ids: vec![],
            video_clip_url: if i % 4 == 0 { Some(format!("https://example.com/goal{}.mp4", i)) } else { None },
        });
    }
    ...
    assert!(disambiguation_time.as_millis() < 100, "Disambiguation context creation should be fast ({}ms)", disambiguation_time.as_millis());
    assert!(processing_time.as_millis() < 200, "Goal event processing should be fast ({}ms)", processing_time.as_millis());
    assert!(format_time.as_millis() < 50, "Name formatting should be fast ({}ms)", format_time.as_millis());
    assert!(total_time.as_millis() < 500, "Total processing should be fast ({}ms)", total_time.as_millis());

    // Verify results are correct
    assert_eq!(processed_events.len(), 20, "Should have processed all 20 goal events");
    assert_eq!(home_disambiguated.len(), 50, "Should have disambiguated all 50 home players");
    assert_eq!(away_disambiguated.len(), 50, "Should have disambiguated all 50 away players");

    // Verify some disambiguation occurred
    let koivu_count = home_disambiguated.values()
        .filter(|name| name.starts_with("Koivu"))
        .count();
    assert!(koivu_count > 1, "Should have multiple Koivu players disambiguated");
    ...
    assert!(away_context.needs_disambiguation("Koivu"), "Should need disambiguation for Koivu");

    println!("✓ Performance impact with large datasets is acceptable");
    println!("  - Disambiguation context: {}ms", disambiguation_time.as_millis());
    println!("  - Goal event processing: {}ms", processing_time.as_millis());
...
async fn test_cross_team_disambiguation_isolation() {
    ...
    assert_eq!(processed_events.len(), 2, "Should have 2 processed goal events");

    let home_koivu_goal = processed_events.iter()
        .find(|event| event.scorer_player_id == 101)
        .expect("Should find home Koivu goal");
    assert_eq!(home_koivu_goal.scorer_name, "Koivu", "Home Koivu should NOT be disambiguated");

    let away_koivu_goal = processed_events.iter()
        .find(|event| event.scorer_player_id == 201)
        .expect("Should find away Koivu goal");
    assert_eq!(away_koivu_goal.scorer_name, "Koivu", "Away Koivu should NOT be disambiguated");
    ...
    assert!(!home_context.needs_disambiguation("Koivu"), "Home team should not need Koivu disambiguation");
    assert!(!away_context.needs_disambiguation("Koivu"), "Away team should not need Koivu disambiguation");

    // Test with format_with_disambiguation directly
    let home_disambiguated = format_with_disambiguation(&home_players);
    let away_disambiguated = format_with_disambiguation(&away_players);

    assert_eq!(home_disambiguated.get(&101), Some(&"Koivu".to_string()), "Home Koivu should not be disambiguated");
    assert_eq!(away_disambiguated.get(&201), Some(&"Koivu".to_string()), "Away Koivu should not be disambiguated");
...
async fn test_unicode_character_handling_in_disambiguation() {
    ...
    assert_eq!(home_disambiguated.get(&101), Some(&"Kärppä Ä.".to_string()), "Should disambiguate with Ä initial");
    assert_eq!(home_disambiguated.get(&102), Some(&"Kärppä Ö.".to_string()), "Should disambiguate with Ö initial");
    assert_eq!(home_disambiguated.get(&103), Some(&"Kärppä Å.".to_string()), "Should disambiguate with Å initial");

    // Verify Mäkelä players are disambiguated
    assert_eq!(home_disambiguated.get(&104), Some(&"Mäkelä M.".to_string()), "Should disambiguate Mäkelä with M initial");
    assert_eq!(home_disambiguated.get(&105), Some(&"Mäkelä V.".to_string()), "Should disambiguate Mäkelä with V initial");

    // Verify Björkström players are disambiguated
    assert_eq!(away_disambiguated.get(&201), Some(&"Björkström J.".to_string()), "Should disambiguate Björkström with J initial");
    assert_eq!(away_disambiguated.get(&202), Some(&"Björkström Å.".to_string()), "Should disambiguate Björkström with Å initial");

    // Verify unique name is not disambiguated
    assert_eq!(away_disambiguated.get(&203), Some(&"Pöllönen".to_string()), "Unique name should not be disambiguated");
    ...
    assert_eq!(unicode_name, "Kärppä Ä.", "Should handle Unicode in first initial");

    let unicode_name2 = format_for_display_with_first_initial("Öljynen", "Kärppä");
    assert_eq!(unicode_name2, "Kärppä Ö.", "Should handle Ö in first initial");

    let unicode_name3 = format_for_display_with_first_initial("Åke", "Kärppä");
    assert_eq!(unicode_name3, "Kärppä Å.", "Should handle Å in first initial");
    ...
    assert!(away_context.needs_disambiguation("Björkström"), "Should need disambiguation for Björkström");
    assert!(!away_context.needs_disambiguation("Pöllönen"), "Should not need disambiguation for Pöllönen");
...
async fn test_edge_cases_and_error_resilience() {
    ...
    assert_eq!(disambiguated.get(&101), Some(&"Koivu".to_string()), "Empty first name should fallback to last name");
    assert_eq!(disambiguated.get(&102), Some(&"Koivu S.".to_string()), "Valid first name should be disambiguated");
    assert_eq!(disambiguated.get(&103), Some(&"Selänne".to_string()), "Unique name should not be disambiguated");
    ...
    assert_eq!(disambiguated2.get(&201), Some(&"Dumont J.".to_string()), "Should use first letter of hyphenated name");
    assert_eq!(disambiguated2.get(&202), Some(&"Dumont M.".to_string()), "Should use first letter of multi-word name");
    assert_eq!(disambiguated2.get(&203), Some(&"González".to_string()), "Unique name should not be disambiguated");
    ...
    assert!(disambiguated3.contains_key(&301), "Should handle numeric first name");
    assert!(disambiguated3.contains_key(&302), "Should handle quoted first name");
    assert!(disambiguated3.contains_key(&303), "Should handle normal first name");

    // Case 4: Very long names
    let players_with_long_names = vec![
        (401, "Verylongfirstnamethatexceedsnormallimits".to_string(), "Verylonglastnamethatexceedsnormallimits".to_string()),
        (402, "Another".to_string(), "Verylonglastnamethatexceedsnormallimits".to_string()),
    ...
    assert!(disambiguated4.contains_key(&401), "Should handle very long names");
    assert!(disambiguated4.contains_key(&402), "Should handle long last names");
    assert!(disambiguated4.contains_key(&403), "Should handle short names");
    ...
    assert!(disambiguated_empty.is_empty(), "Empty player list should return empty result");
    ...
    assert_eq!(disambiguated_single.get(&501), Some(&"Koivu".to_string()), "Single player should not be disambiguated");

    // Case 7: Test DisambiguationContext with edge cases
    let context_empty = DisambiguationContext::new(vec![]);
    assert!(!context_empty.needs_disambiguation("AnyName"), "Empty context should not need disambiguation");

    let context_single = DisambiguationContext::new(single_player);
    assert!(!context_single.needs_disambiguation("Koivu"), "Single player context should not need disambiguation");
